...
8. [Holiday Adapter Specific Patterns](#holiday-adapter-specific-patterns)
...
This is the **feiertage** (German Holidays) adapter for ioBroker. This adapter calculates and tracks religious, political, and calendar holidays for Austria (AT), Switzerland (CH), Germany (DE), and Italy (IT). The adapter runs on a schedule (daily at midnight) and creates state objects for:

- Current day holiday status (heute/today) 
- Tomorrow's holiday status (morgen/tomorrow)
- Day after tomorrow's holiday status (uebermorgen/aftertomorrow)
- Next upcoming holiday information (naechster/next) with name, date, and duration

Key features include:
- Easter calculation for variable religious holidays
- Advent calculation for Christmas season
- Leap year handling
- Multi-language support (German/English state names)
- Holiday selection configuration through admin interface
- Support for regional variations across countries

The adapter uses complex date calculations including Easter algorithms, Advent calculations, and handles edge cases like year transitions and leap years. It follows the ioBroker schedule pattern where it runs calculations and immediately stops, rather than maintaining a persistent connection.
...
## Holiday Adapter Specific Patterns

### Holiday Calculation Specifics
- Use consistent date handling throughout the adapter
- Ensure calculations work across year boundaries
- Handle regional holiday variations properly
- Maintain accurate Easter and Advent calculations
- Consider timezone implications for midnight scheduling

### Configuration Management
- Validate holiday selections from admin interface
- Handle missing or invalid configuration gracefully
- Preserve user preferences during updates
- Provide clear descriptions for holiday options

### State Management Pattern
```javascript
// Create parallel German/English states for compatibility
this.setState('heute.Name', {ack: true, val: holidayNameDE});
this.setState('today.name', {ack: true, val: holidayNameEN});
this.setState('heute.boolean', {ack: true, val: !!holiday});
this.setState('today.boolean', {ack: true, val: !!holiday});
```

### Date Calculation Algorithms
- Use centralized holiday definition from `admin/holidays.js`
- Calculate based on day of year for efficiency
- Handle special cases (Easter, Advent) with dedicated algorithms
- Provide both immediate and future holiday information

### Schedule Management
- Respect the daily midnight schedule for calculations
- Handle adapter restarts gracefully
- Ensure calculations complete before adapter stops
- Log calculation progress appropriately

### Multi-language Support
- Support German and English language variants
- Use proper translation keys for admin interface
- Handle language fallbacks appropriately
